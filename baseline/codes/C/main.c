...
int main(){
    ...
    Parms_.kcross1 = (REAL *) malloc(N_AGENTS*sizeof(REAL));
    InitK(&Parms_);
    ...
    while(metric_coeff>TOLL_COEFF){
        ...
        if (metric_coeff>TOLL_COEFF*100){
            ...
            for(int j=0;j<N_AGENTS;j++)
                Parms_.kcross[j] = Parms_.kcross1[j];
        }
        #endif
    ...
    // Yes, these are \n separated and not CSV
    // but current R script is hard-coded to CSV extensions for convenience
    // because the Matlab file writer outputs to CSV files.
    // In future, could maybe change Matlab to output \n separated, or swap this to true CSV output
    // (But easier to examine in text editor if \n separated)
#if 1
    FILE *fp;
    fp = fopen("./kprime.csv", "w");
    #if USE_FLOAT
    char print_line[10] = "%.7f\n";
    #else
    char print_line[10] = "%18.15lf\n";
    #endif

    for (int is = 0; is < NSTATES; ++is) {
        fprintf(fp, print_line, (double) Parms_.kprimen[is]);
    }
    fclose(fp);

    fp = fopen("./kcross.csv", "w");
    for (int is = 0; is < N_AGENTS; ++is) {
        fprintf(fp, print_line, (double) Parms_.kcross[is]);
    }
    fclose(fp);

    fp = fopen("./r2bg.csv", "w"); // Order: bad, good
    for (int is = 0; is < 2; ++is) {
        fprintf(fp, print_line, (double) Parms_.R2[is]);
    }
    fclose(fp);

    fp = fopen("./coeffs.csv", "w"); // Hopefully the order here is same as in Matlab
    for (int is = 0; is < 4; ++is) {
        fprintf(fp, print_line, (double) Parms_.coeff[is]);
    }
    fclose(fp);
#endif
    printf(">>>>> DONE <<<<<\n");
