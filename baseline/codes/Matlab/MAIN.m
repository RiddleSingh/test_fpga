...
function [kp, kcross, B, runinfo] = MAIN(ngridk, ngridkm, interp_type, n_agents, verbose)
    %__________________________________________________________________________
    %
 ...
 %   N=10000;          % number of agents for the stochastic simulation
    ...
    dlmwrite('./idshock.txt',idshock,'delimiter','\t');
    dlmwrite('./agshock.txt',agshock,'delimiter','\t');
    %__________________________________________________________________________
    %
    % Grids
    %__________________________________________________________________________
...
%    ngridkm=4;                           % number of grid points for km 
    ...
    egm_niters = zeros(1,1000);
    egm_timetots = zeros(1,000);

    %% Main convergence loop
    i=1; 
    while dif_B>criter_B % perform iterations until the difference between 
                          % coefficients is less or equal than criter_B

        [kprime,c,niter, timetot]  = INDIVIDUAL(interp_type, prob,ur_b,ur_g,ngridk,ngridkm,nstates_ag,nstates_id,k,km,er_b,er_g,a,epsilon,l_bar,alpha,delta,gamma,beta,mu,km_max,km_min,kprime,B,criter_k,k_min,k_max,update_k);
                    % compututing a solution to the individual problem
        egm_niters(iteration+1) = niter;
        egm_timetots(iteration+1) = timetot;

        if Method==1; 
            [kmts,kcross1]  = AGGREGATE_ST(interp_type, T,idshock,agshock,km_max,km_min,kprime,km,k,epsilon2,k_min,k_max,kcross,a2);
        else % i.e., Method 2 
            [kmts,kcross1]  = AGGREGATE_NS(interp_type, l_bar,alpha,prob,ur_b,ur_g,T,J,kvalues_min,kvalues_max,ngridk,ngridkm,nstates_ag,nstates_id,idshock,agshock,km_max,km_min,kprime,km,k,epsilon2,ndiscard,k_min,k_max,kcross,a,a2);
        end
        ...
        if verbose
            fprintf('Iter %d: Spread %f, %f %f %f %f\n',iteration,dif_B,B(1),B(2),B(3),B(4));
        end
        
        dlmwrite(strcat("golden__2K1K_kp_", num2str(iteration), ".dat"), kprime);
        dlmwrite(strcat("golden__2K1K_kc_", num2str(iteration), ".dat"), kcross);
    ...
    runinfo.R2bad = R2bad;
    runinfo.R2good = R2good;
    runinfo.main_iters = iteration;
    runinfo.egm_avg_iters = mean(egm_niters(1:iteration));
    runinfo.egm_avg_timetot = mean(egm_timetots(1:iteration));
    runinfo.egm_avg_time_iter = mean(egm_timetots(1:iteration) ./ egm_niters(1:iteration));
    runinfo.main_avg_time_iter = iteration / et;

    format long g; 
    disp('R^2 bad aggregate shock:'); R2bad(1)
    disp('R^2 good aggregare shock:'); R2good(1)
    disp('Average EGM niter:'); mean(egm_niters(1:iteration))
    disp('Average EGM timetot:'); mean(egm_timetots(1:iteration))
    disp('Average EGM time per iteration:'); mean(egm_timetots(1:iteration) ./ egm_niters(1:iteration))
    format;
    %__________________________________________________________________________
    %
