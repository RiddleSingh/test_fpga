...
void init_all(env_t *env, input_t *in, vars_t *vars)
{
	...
	in->idshock = (ids_sw_t *) mem_alloc(IDSHOCK_ARR_SIZE);
	in->agshock = (unsigned char *) mem_alloc(AGSHOCK_ARR_SIZE);
	vars->kprime_a  = (real *) mem_alloc(NSTATES*sizeof(real));
	vars->kprime_b = (real *) mem_alloc(NSTATES*sizeof(real));
	vars->kcross_a = (real *) mem_alloc(N_AGENTS*sizeof(real));
	vars->kcross_b = (real *) mem_alloc(N_AGENTS*sizeof(real));
	vars->coeff = (real *) mem_alloc(NCOEFF*sizeof(real));
	vars->coeff1 = (real *) mem_alloc(NCOEFF*sizeof(real));
	vars->kmprime = (real *) mem_alloc(NSTATES_AG*NKM_GRID*sizeof(real));
	vars->kmts  = (real *) mem_alloc(SIM_STEPS*sizeof(real));
...
void free_all(input_t *in, vars_t *vars)
{
	mem_free(in->idshock);
	mem_free(in->agshock);
	mem_free(vars->kprime_a);
	mem_free(vars->kprime_b);
	mem_free(vars->kcross_a);
	mem_free(vars->kcross_b);
	mem_free(vars->coeff);
	mem_free(vars->coeff1);
	mem_free(vars->kmts);
	mem_free(vars->kmprime);
	return;
}
