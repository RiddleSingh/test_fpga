function arg_out = MMV(ngridk_external,ngridkm_external,OUTPUTFOLDER)
% This function adapts the main file from Maliar, Maliar, Valli (2010)  to
% automatize the solution of the model for multiple grid sizes:
%{
Inputs:
ngridk_external: individual capital holdings grid size
ngridkm_external: aggregate capital grid size
OUTPUTFOLDER: folder where to store results
%}
%%%%%%%%
...
ngridk=ngridk_external;                % number of grid points
...
ngridkm=ngridkm_external;                           % number of grid points for km 
...
%__________________________________________________________________________
%
save([OUTPUTFOLDER,'/MMV/Solution_to_model_nKM',num2str(ngridkm),'-nk',num2str(ngridk),'.mat']);
